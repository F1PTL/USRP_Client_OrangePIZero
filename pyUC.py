...
#**************************************************************
#       Pour trouver les modifs aplliquees  tapez MODIF       *
...
#   t3="t3.txt="
#   x=t3+'"'+str(rxslot)+'"'  # pas necessaire pour l'instant 
#   b=x.encode()              # idem
#   sendToNextion(b)          # idem
...
#  affiche le mode   
# voir MODIF loggin(getMaster(mode))
def getMode(mode):
    
   if mode =='DSTAR':
         sendToNextion("page 1".encode())   # conversion en bytes page 1= DSTAR
   if mode =='DMR':
         sendToNextion("page 2".encode())   # idem page 2=DMR
   if mode=='YSF':
         sendToNextion("page 3".encode())   # idem page 3=YSF
   if mode=='P25':
         sendToNextion("page 4".encode())   # idem page 4=P25 
   if mode=='NXDN':
         sendToNextion("page 5".encode())   # idem page 5=NXDN
...
# p2 est la picture du point dans le nextion
def pttOff():
...
def receive():
   ...
   b=rec.encode()
   sendToNextion(b)
   #GPIO.output(RECEPT ,GPIO.HIGH)    # apres test inutile
   return
            ...
            if (type == USRP_TYPE_VOICE): # voice
                ...
                if (audio[0:4] == REG):
                    ...
                    if audio[0] == TLV_TAG_SET_INFO:
                        ...
                        receive()    # MODIF
                        getForNextion(call,rxslot,tg,mode,name)
...
#                    decay = vox_delay.get()     # Yes, reset the decay value (wont unkey for N samples)
  ...
  ###########################
            ...
            if ptt != lastPtt and GPIO.input(PTT)==0:   # MODIF
                usrp = 'USRP'.encode('ASCII') + struct.pack('>iiiiiii',usrpSeq, 0, ptt, 0, USRP_TYPE_VOICE, 0, 0) + audio
                sendto(usrp)
                usrpSeq = usrpSeq + 1
            lastPtt = ptt
            if ptt == True and GPIO.input(PTT)==1:    # MODIF
                usrp = 'USRP'.encode('ASCII') + struct.pack('>iiiiiii',usrpSeq, 0, ptt, 0, USRP_TYPE_VOICE, 0, 0) + audio
                sendto(usrp)
                usrpSeq = usrpSeq + 1
                #audio_level.set(int(rms/100)) #MODIF
        except:
...
def showPTTState(flag):
    ...
    if ptt and GPIO.input(PTT)==1:   # MODIF
        pttOn()   # MODIF
