...
##################################################################################""
   ...
   # ############################################################
   #                                                            #
   #        Modifications ON4RD & Bruno F1PTL                   #
   #  F1PTL  concepteur de l image de DVSWITCH                  #
   #         et dvswitch mobile (android)                       #
   #  Avec un hotspot (qui peut etre un tx analogique)          #
   #   vous  faites tous les modes digitaux et en plus          #
   #      l'analogique "Echolink" et le "RRF" de  F5NLG         #
   #  avec l'orange pi et le super travail de Bruno F1PTL       #
   #   Merci aux concepteurs du logiciel  N4IRR et ,N4IRS       #
   #                                                            #
   #                Ecran Nextion :                             #
   #          Nouveau style  en construction                    # 
   #******************************************************** ***#
   #                                                            #
   #   le HMI  du Nextion sera dans le fichier compresse        #
   #                                                            #
   #          gpioMode  automatique                             #
   #                                                            #
   #   diminution automatique du gain de la modulation          #
   #          sorte d agc  pour tous les modes (test)           #
   #  Merci a tous ceux qui participent  au  projet             #
   #************************************************************#
   #                                                            #
   #  Pour trouver les modifs tapez MODIF dans votre recherche  #
   #                                                            #
   ##############################################################
...
#  NOTE   (call,rxslot,tg,loss,time()-start_time)
#  infos du log a afficher sur l ecran 
# T0 a T8  sont desvariables du nextion   t0.txt="10208"
#*****************************************************************************


def getForNextion(call,name,rxslot,tg,loss,rxTime):
   import time
   makeLog(time.ctime(),call,name,tg,loss,rxTime)
   t0="t0.txt="               # t1.txt= "valeur"   
   x=t0+'"'+str(call)+'"'
   b=x.encode()               # convertit les chaines en bytes  
   sendToNextion(b)           # puis envoie le resultat sur l ecran  
   t1="t1.txt="               # idem ci-dessous mais pour le TG 
   x=t1+'"'+str(tg)+'"'       #   ajout du TG a  t1
   b=x.encode()               #  repetition de la routine 
   sendToNextion(b)           # 
   t3="t3.txt="               # 
   x=t3+'"'+str(loss)+'"'     # perte de paquets  dans la transmission 
   sendToNextion(b)           # envoyer sur l'ecran
   t4="t4.txt="
   x = t4+'"'+str(name)+'"'
   t2="t2.txt="
   x = str(rxTime)            #  duree de la transmition rx et tx 
   x=t2+'"'+str(x[:4])+'"'
   b=x.encode()
   sendToNextion(b)
   # heure locale   pour GMT,UTC  time.gmtime()
   t=time.ctime()              # ajout de l heure locale
   t8="t8.txt="
   x= t8+'"'+str(t)+'"'
   b=x.encode()
   sendToNextion(b)
      
   return call,name,rxslot,tg,loss,rxTime
 # rNextion(call,prenom,rxslot,tg,loss,rxTime): 



# Envoie les infos du log sur ecran Nextion
def sendToNextion(cmd):
#   ser.write(cmd)             # contenu  de la variable 
#   ser.write(EndCom)          # cloture de la commande  
   return cmd
...
#***************************************************************************
def getSetMode(mode,flag): #  flag verifie si c'est un code dtmf ou pas
#    ici on affiche le logo du mode
    global _mode  
    global flag2
    _mode = mode 
    logoMode(mode) 
...
def agcMicGain(db):    # test un agc pour tous les mmodes
    global niveaux
     
    niveaux  = 200       # gain max  audio_level
    gain_ref = 2        # gain reference du rms
    gain_max = gain_ref  #  on mets tout au meme niveau
    gain_min = gain_ref  # idem
    gain_actuel = calc.log10(db) # on calcal les db's
     

    if gain_actuel > gain_ref :    # agc est en fonction
        gain_max += 1              #  
        sp_vol.set(gain_max)       # valeur stockee dans le sp
        mic_vol.set(gain_max)      # et dans le mic 
        gain_max /=10              #  en divisant le gain on augmentela modul.
        niveaux = gain_max         # on le met dans niveau rms
    else: 
        gain_min  -= 1             # idem mais l inverse
        sp_vol.set(gain_min)       # on fait la meme chose avec le hp 
        mic_vol.set(gain_min)      # et le mic 
        gain_min *= 10             # plus on multiplie et plus le gain diminue
        niveaux = gain_min         # on stocke le gain dan s niveau
    return db                      # en principe plus on parle pres du mic 
                                   # et plus le gain diminue 
                                   # idem pour la reception 

#******************************************************************************$
# sur l'ecran il y a un temoin rouge pour le ptt et vert pour la reception
# p2 est la picture du rouge dans le nextion
def pttOn():
   global _mode               # le mode digital recupere dans une variable G 
   logoMode(_mode)             # logo  ex:P25   destination Nextion 
   ptt="vis p2,1"              # si  1 le rouge s'allume 
   b=ptt.encode()              # encode en bytes
   sendToNextion(b)            #  envoyer le resultat sur l'ecran
   return 

# Fin de transmission on lache le ptt et le  rouge 
# s'eteint vis est a 0
# p2 est la photo txrx du nextion
def pttOff():
   ptt="vis p2,0"             # p2 : ptt  rouge sur Nextion
   b=ptt.encode()             # conversion chaine en byte
   sendToNextion(b) 
   return
#*****************************************************************************
# On utilise le GPIO "mode pin" n0 26 ou celui de votre choix pour 
#    commuter le hotspot en TX pour lae reception sur le hotspot  
# ########################################################################"
def receive():
   global SQUELCH
   GPIO.output(SQUELCH ,GPIO.HIGH)  #  commutation du GPIO en etat haut pour le squelch  
   rec="vis p1,1"                  #  vis est a 1 donc on allume le  vert
   b=rec.encode()                  #  lorsqu il  recoit une trame
   sendToNextion(b)
   return

# On met le GPIO recepetion a l'etat bas
#  pour arreter la transmission du hotspot  
#############################################################"
def noReceive():
   global _mode 
   global SQUELCH                     #  le mode est recupere 
   nrec="vis p1,0"                   #  on eteint le temoin de reception
   b=nrec.encode()
   sendToNextion(b)
   GPIO.output(SQUELCH ,GPIO.LOW)     # on arrete le squech donc le hotspot
   return

#***************************************************************************

#******************************************************************************$
# NOTE makeLog(time.ctime(),call,tg,loss,rxtime)
# c'est ici que l'on ecrit dans le log # le fichier mylog.dvs
def makeLog(tc,call,name,tg,loss,rxTime):
   global _mode
   global _call   
   txTime=''
   mode = _mode  #master.get()           # on acquis le mode
   txTime=str(rxTime)            # idem temps de transmission
   duration=str(txTime[:4])      # duree de transmission  
   myLog=[tc,call,name,tg,mode,loss,duration]    # table du log
   log=open('mylog.dvs','a')     # ecris a la fin du fichier
   log.write(str(myLog[:6]))       #  add log dans le fichier mylog.dvs
   log.write('\n')                 # suivi d'un return carrier 
   log.close()                        # on ferme le log
   return tc,call,name,tg,loss,rxTime  

#*****************************************************************************
def barGraphTxRx(val): # val est audio=(int rms /100) 
      global _mode 
      logoMode(_mode)                 #  vrifie si le mode a change
      vumetre (val)                   # S-metre  a aiguille sur nextion
      return val


######################################################################

def sendRmsToNextion(rms): #  envoie le rms sur le Nextion
     global ptt
     if ptt != True:
       barGraphTxRx(rms)   # sous forme de bargraph  
       return rms
     else:
       barGraphTxRx (rms/100)  #   pour respecter l'echelle graphique  
     return rms 

##########################################################################
def vumetre(val):   # nouveau pour le Nextion en preparation
     
    if val > 160 :   # max de l'aiguille
        val = 160    # si plus de alors max=160  
    
    z0="z0.val="+str(val)    # z0 est la variable de l aiguille
    b= z0.encode()           
    sendToNextion(b) 
    return val
##########################################################################"
def pageNbr(n):         # la page sur laquelle on envoi les donnees
    p="page "+str(n)    
    b=p.encode()
    sendToNextion(b)
    return n 
...
STRING_GPIO_MODE = "Inv Gpio" 
STRING_TX_ENABLE = "OK Transmit"
STRING_VOX_THRESHOLD = "Gpio PTT "
STRING_VOX_DELAY = "Gpio squelch"
...
STRING_RESERVE = "Reserve"
STRING_LOG = "Call & Name"
STRING_VERS = "v b06.751"
STRING_NAME = "Name" 
###################################################################################
...
def openStream():
    ...
    try:
        ...
        pass
    udp.bind(('', usrp_rx_port))  #  MODID  a voir plus tard

def sendto(usrp):
    for port in usrp_tx_port:    # MODIF A voir plus tard
...
def log_end_of_transmission(call,rxslot,tg,loss,start_time):
    ...
    logging.info('End TX:    {} {} {} {} {:.2f}s'.format(str(call)+','+str(name),rxslot, tg, loss, time() - start_time))
    noReceive()            # MODIF
    logList.see(logList.insert('', 'end', None, values=(
        strftime("%d/%m/%y", localtime(start_time)),
        strftime("%H:%M:%S", localtime(start_time)),
        call+','+str(name),rxslot, tg, loss, '{:.2f}s'.format(time() - start_time)))) 
            ...
            if (type == USRP_TYPE_VOICE): # voice
                ...
                if (len(audio) == 320):
                    if RATE == 48000:
                        ...
                        barGraphTxRx(int(rms/100))  # MODIF
                    else:
                ...
                if (keyup != lastKey):
                    ...
                    if keyup:
                        start_time = time()
                    if keyup == False:
                        getForNextion(call,name,rxslot,tg,loss,time()-start_time) # MODIF
                        #call = str(call)+str(name)       # MODIF
                        log_end_of_transmission(call+','+str(name), rxslot, tg, loss, start_time)
                ...
                if (audio[0:4] == REG):
                    ...
                    if audio[0] == TLV_TAG_SET_INFO:
                        if transmit_enable == False:    #EOT missed?
                            #call=str(call)+'"','"'+str(name)
                            log_end_of_transmission(call+','+str(name), rxslot, tg, loss, start_time)
                            getForNextion(call,name,rxslot,tg,loss, start_time)
                        ...
                        logging.info('Begin TX: {} {} {} {} {}'.format(call,name, rxslot, tg, mode))
                        receive()    # MODIF
                        getForNextion(call,name,rxslot,tg,loss,time()-start_time) #MODIF
...
#                logging.debug(audio[:audio.find('\x00')])
             ...
             #print ("test")  # logging.info(soundData, len(soundData))
            pass
...
def txAudioStream():
    ...
    while done == False:
        try:
            ...
            rms = audioop.rms(audio, 2)  # MODIF  Get a relative power value for the sample
            
            
            sendRmsToNextion(rms)
             
            if txEnable.get():
                    #pttState=GPIO.input(PTT) 
                    agcMicGain(rms)  #vox_threshold.get() # vox_delay.get()  Ye$
                    if ((ptt == False)  and (transmit_enable == True) and (GPIO.input(PTT) ==EtatA)): # MODIF
                         ptt = True              # Set it
                         showPTTState(0)         # Update the UI (turn transmit$
                         pttOn()                 # MODIF
                    elif ptt == True and GPIO.input(PTT)==EtatB :  #  MODIF   # Are we$
                         ptt = False             # Unkey
                         showPTTState(1)         # Update the UI
                         pttOff()                # MODIF

            if ptt != lastPtt and GPIO.input(PTT) == EtatB :  # MODIF
                usrp = 'USRP'.encode('ASCII') + struct.pack('>iiiiiii',usrpSeq, 0, ptt, 0, USRP_TYPE_VOICE, 0, 0) + audio
                sendto(usrp)
                usrpSeq = usrpSeq + 1
            lastPtt = ptt
            if ptt == True and GPIO.input(PTT) == EtatA :   # MODIF
                usrp = 'USRP'.encode('ASCII') + struct.pack('>iiiiiii',usrpSeq, 0, ptt, 0, USRP_TYPE_VOICE, 0, 0) + audio
                sendto(usrp)
                usrpSeq = usrpSeq + 1
                audio_level.set(int(rms/niveaux)) # MODIF
                barGraphTxRx(int(rms/100))  # MODIF 
...
def setAudioData():
    ...
    #gpios="true" if dongle_mode.get() > 0 else "false"  # MODIF DONGLE
...
def getValuesFromServer():
    ...
    getVoxData()        # MODIF vox enable, delay and threshold
    dongle_mode.set(1)                   #dongle mode enable
    
    mic_vol.set(40) # 50   modif                  #microphone level
...
def showPTTState(flag):
    ...
    if ptt == True   : #GPIO.input(PTT)==0:   # MODIF
        pttOn()   # MODIF
...
def makeLogFrame( parent ):
    ...
    cols = (STRING_DATE, STRING_TIME, STRING_LOG, STRING_SLOT, STRING_TG, STRING_LOSS, STRING_DURATION)
    widths = [85,85,80,55,140,70,55]
...
def makeVoxSettingsFrame( parent ):
    ypad = 4
    voxSettings = LabelFrame(parent, text=STRING_VERS, padx=5, pady = ypad, fg=uc_text_color, bg = uc_background_color, relief = SUNKEN) 
    Checkbutton(voxSettings, text = STRING_TX_ENABLE, variable=txEnable, command=lambda: cb(txEnable), fg=uc_text_color, bg = uc_background_color, bd = 0, highlightthickness = 0).grid(column=1, row=2, sticky=W)
    #Checkbutton(voxSettings, text = STRING_GPIO_MODE , variable=gpioState, command=lambda: invGpioState(gpioState.get()), fg=uc_text_color, bg = uc_background_color, bd = 0, highlightthickness = 0).grid(column=2, row=4, sticky=W) 
    #whiteLabel(voxSettings, STRING_VOX_DELAY).grid(column=1, row=4, sticky=W, padx = 5, pady = ypad)
    #Spinbox(voxSettings, from_=1, to=40, width = 5, fg=uc_text_color, bg=uc_background_color, textvariable = vox_delay).grid(column=2, row=4, sticky=W, pady = ypad)
...
def makeSettingsFrame( parent ):
    ...
    #makeLogCallFrame(settingsFrame).grid(column=1,row=8, sticky=(N,W), padx = 5, pady = 5) #, columnspan=2)
...
if len(sys.argv) > 1:
    ...
    config_file_name = str(Path(sys.argv[0]).parent) + "/pyUCb6.77.ini"       # Use the default config file name in the same dir as .py file
...
#    dongle_mode = makeTkVar(IntVar, config.get('DEFAULTS', "dongleMode").split(None)[0]) # MODIF
    gpioState= makeTkVar(IntVar, config.get('DEFAULTS', "gpioState").split(None)[0])
    mic_vol = makeTkVar(IntVar, config.get('DEFAULTS', "micVol").split(None)[0])
    txEnable = makeTkVar(IntVar, config.get('DEFAULTS', "txEnable").split(None)[0])
...
#    vox_delay = makeTkVar(IntVar, config.get('DEFAULTS', "voxDelay").split(None)[0])
    pinSquelch = makeTkVar(IntVar, config.get('DEFAULTS', "pinSquelch").split(None)[0])
    pinPtt = makeTkVar(IntVar, config.get('DEFAULTS', "pinPtt").split(None)[0])    
    ...
    getSetMode(defaultServer,0)     # MODIF
    asl_mode = makeTkVar(IntVar, config.get('DEFAULTS', "aslMode").split(None)[0])
    useQRZ = bool(readValue(config, 'DEFAULTS', 'useQRZ', False, int)) #MODIF True
    ...
    PTT= pinPtt.get()
    SQUELCH = pinSquelch.get()
    stateGpio = gpioState.get()
    if stateGpio  == 0:
        from  RPi import GPIO 
        GPIO.setmode(GPIO.BOARD) 
        GPIO.setwarnings(False) 
        GPIO.setup(PTT, GPIO.IN, pull_up_down=GPIO.PUD_UP)  
        GPIO.setup(SQUELCH,GPIO.OUT)
        GPIO.output(SQUELCH,GPIO.LOW)   # par defaut la reception est desacivee
        EtatA = 0
        EtatB = 1
    else: 
        import OPi.GPIO as GPIO
        GPIO.setboard(GPIO.ZERO) # Ajout pour l OrangePIZero 
        GPIO.setmode(GPIO.BOARD) # mode pin  ou GPIO.BCM mode GPIO
        GPIO.setwarnings(False)
        GPIO.setup(PTT, GPIO.IN, pull_up_down=GPIO.PUD_DOWN) #  on active la res$
        GPIO.setup(SQUELCH, GPIO.OUT)
        EtatA = 1
        EtatB = 0
...
#makeLogCallFrame(root).grid(column=1, row=2)         # DERNIERE MODIF
makeStatusBar(root).grid(column=1, row=6, sticky=W+E)
