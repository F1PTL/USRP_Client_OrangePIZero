...
import math as calc
trigger = 0
PTT = 31                  # pin 31 du raspi = GPIO 06
RECEPT = 29 # pin 29 du raspi  
flag2=0
...
#  a alijamaliz  pour son super logiciel                      *
#        DTMF-DETECTOR dont voici  le lien  ci-dessous        *
#    https://github.com/alijamaliz/DTMF-detector              *
#          ajout d'un log simple                              *
#    ajout pour le nextion tapez sur l ecran et               *
#          sa luminosite diminuera  et vice versa             *
#        ajout DTMF dans un autre py                          *
#                 il faut 2 cartes sons                       *
#  utilisation du vox modifie avec <<ctcss>>  pour le ptt     *
#    reste un petit probleme de rebond au ptt off             *
#  ajout:  vox_delay du gui dans le pyUC.ini                  *
#         si > 2   GPIO  par defaut                           *
#         EtatA = 0  EtatB =  0                               *
#         si = 2 EtatA = 1 EtatB = 0                          *
#         peut-etre devoir changer le pull_up  en down        *
#  ajout de la barre horizontale de couleur de la modulation  *
#  sur l'ecran nextion  comme sur USRP rouge(tx) vert(rx)     *
...
def ptt_On_Off(x):
    ...
    if dongle_mode.get()!= False:
        pttAudio = GPIO.input(PTT) # ici  pttAudio = GPIO
        #print ("EtatA :",EtatA,"Etatb :",EtatB) # debug 
        return x 
    else:
        #print ("EtatA :",EtatA,"Etatb :",EtatB)  # debug
        diff=comp(x,trigger)       # ici on utilise pas le GPIO mais EtatA(B)
    return x
    

#************************************************************************************    
      
def inverseGpioState(State_2):          # si vox_delay = 1 alors par defaut sinon inverse  
    global EtatA                 #    pttAudio = EtatA
    global EtatB                 #    pttAudio = EtatB
    if State_2 == 1:
       EtatA = 0                 #    GPIO.input(PTT)=EtatA 
       EtatB = 1                 #    GPIO.input(PTT)=EtatA 
    if state_2 == 2:
       EtatA = 1                 #    GPIO.input(PTT)=EtatB  
       EtatB = 0                 #    GPIO.input(PTT)=EtatA 
    else:
       EtatA= EtatA
       EtatB= EtatB 
    return state_2
...
def gpioOnOff(flag):
    global pttAudio              # remplace le ptt_vox par le GPIO
    global gpioMode              # si la case dongle_mode est cochee  
                                 #  aloars on passe en mode  GPIO(PTT)
    d = flag                     # flag est la val du  la case dongle_mode
    #print ("d =",d)             # debug
    if d == 1: 
        gpioMode = True          #  feu vert pour le mode GPIO
    else:
        gpioMode = False         #  on n utilise pas le GPIO
...
EndCom=b"\xff\xff\xff"        # fin de la commande d envoie vers l ecran 

#  Le nextion demande toujours une fin de commande 
# ici ca peut-etre yyy avec "trema" ou 255 255 255 ou 0xFF 0xFF 0xFF
#  NOTE   (call,rxslot,tg,loss,time()-start_time)
#  infos du log a afficher sur l ecran 
# T0 a T4  sont des variables du nextion   t0.txt="10208"
#*****************************************************************************************

def getForNextion(call,rxslot,tg,loss,rxTime):
   import time
   makeLog(time.ctime(),call,tg,loss,rxTime) # ici on  ecrit dans le log
   t1="t1.txt="               # t1.txt= "valeur"   
   x=t1+'"'+str(call)+'"'     # valeur =  les calls TX et RX
   b=x.encode()               # convertit les chaines en bytes  
   sendToNextion(b)           # puis envoie le resultat sur l ecran  
   t0="t0.txt="               # idem ci-dessous mais pour le TG 
   x=t0+'"'+str(tg)+'"'       #   ajout du TG a  t0
   b=x.encode()               #  repetition de la routine 
   sendToNextion(b)           # 
   t3="t3.txt="               # 
   x=t3+'"'+str(loss)+'"'     # perte de paquets  dans la transmission 
   b=x.encode()               # idem ci-dessus
   ...
   b=x.encode()
   sendToNextion(b)
                               # ajout de l'heure locale 
                               # On peut changer pour time.gmtime()
   t=time.ctime()              # ajout de l heure locale
   t4="t4.txt="
   x= t4+'"'+str(t)+'"'
   b=x.encode()
   sendToNextion(b)
   return call,rxslot,tg,loss,rxTime
...
#  affiche le mode   
# et chande de mode par dtmf
# exemple P25 = 4#
#******************************************************************************************

def getSetMode(mode,flag): #  flag verifie si c'est un code dtmf ou pas
#    ici on tourne la page vers le mode desire
    p ="page "+str(mode)    #  code a envoyer page x 
    b = p.encode()          # convertis en bytes
    if flag != 0 :          # si 0 alors c'est le dtmf sinon c'est le programme
          os.system("sudo /opt/Analog_Bridge/dvswitch.sh mode " + mode) 
          sendToNextion(b)
    else:
          sendToNextion(b)
...
def agcMicGain(db):    # test un agc pour tous les mmodes
    ...
    gain_max = gain_ref
    gain_min = gain_ref
    gain_actuel = calc.log10(db)
    if gain_actuel > gain_ref : 
        gain_min += 1  
        sp_vol.set(gain_min)
        gain_min = gain_min /10
        niveaux = gain_min
    else: 
        gain_max  -= 1
        sp_vol.set(gain_max)
        gain_max = gain_max *10
        niveaux = gain_max
    return db  

#**************************************************************************************
# sur l'ecran il y a un point rouge pour le ptt et vert pour la reception
# p2 est la picture du point rouge dans le nextion
def pttOn():
   ptt="vis p2,1"      # si vis est a 1 le point rouge s'allume 
   b=ptt.encode()
   sendToNextion(b)
   return 

# Fin de transmission on lache le ptt et le  point rouge 
# s'eteint vis est a 0
# p2 est la photo txrx du nextion
def pttOff():
   reset_Bar()    #   global pttAudio 
   ptt="vis p2,0"             # p2 = eteintle rouge 
   b=ptt.encode()               # conversion chaine en byte
   sendToNextion(b) 
   return
#****************************************************************************************

# On utilise le GPIO 29 ou celui de votre choix pour 
# commuter le hotspot en mode reception 
# et en meme temps on allume le point vert ce qui permet de parler sans pc
def receive():
   GPIO.output(RECEPT ,GPIO.HIGH)  #  commutation du GPIO en etat haut  
   #pttAudio = False
   rec="vis p1,1"                  #  vis est a 1 donc on allume le point vert
   b=rec.encode()
   sendToNextion(b)
   return

# ici le point vert s'eteint on est en stand bye
# et en meme temps on met le GPIO recepetion a l'etat bas
# pour commuter le hotspot en tx ... 
def noReceive():
   nrec="vis p1,0" 
   b=nrec.encode()
   sendToNextion(b)
   GPIO.output(RECEPT ,GPIO.LOW)
   return

#******************************************************************************************
# NOTE makeLog(time.ctime(),call,tg,loss,rxtime)
# c'est ici que l'on ecrit dans le log # le fichier mylog.dvs
def makeLog(tc,call,tg,loss,rxTime):
   txTime=''
   mode = master.get()           # on acquis le mode
   txTime=str(rxTime)            # idem temps de transmission
   duration=str(txTime[:4])
   myLog=[tc,call,tg,mode,duration]  
   log=open('mylog.dvs','a')     # ecris a la fin du fichier
   log.write(str(myLog[:5]))
   log.write('\n')
   log.close()
   return tc,call,tg,loss,rxTime  

#*****************************************************************************************
def bartxrx(val,color): # val est audio=(int rms /100) 
   if color ==1:
       j="j0.pco=59394"  #  rouge pour tx
       b=j.encode()
       sendToNextion(b)  
       sendBar(val) 
       return val,color
   if color == 0:
       j="j0.pco=1024"   # vert pour rx 
       b=j.encode()
       sendToNextion(b)
       sendBar(val)
       return val,color
...
def sendBar(val): # envoie le signal de modulation sur Nextion
     j="j0.val="
     x=j+str(val) 
     b=x.encode()
     sendToNextion(b)
...
def reset_Bar():   # reset barre txrx 
    j="j0.val=0" 
    b=j.encode()
    sendToNextion(b)
    return
...
def sendRmsToNextion(rms): #  envoie le rms sur le Nextion
     global ptt
     if ptt != True:
       bartxrx(rms,0)   # sous forme de bargraph  
       return rms
     else:
       bartxrx (rms/100,1)
       return rms 
            ...
            if (type == USRP_TYPE_VOICE): # voice
                ...
                if (keyup != lastKey):
                    ...
                    if keyup:
                        start_time = time()
                    if keyup == False:
                        getForNextion(call,rxslot,tg,loss,time()-start_time) # MODIF
                ...
                if (audio[0:4] == REG):
                    ...
                    if audio[0] == TLV_TAG_SET_INFO:
                        ...
                        receive()    # MODIF
                        getForNextion(call,rxslot,tg,mode,name) #MODIF
...
def txAudioStream():
    ...
    while done == False:
        try:
            ...
            #print (vox_delay.get())              ###### Vox processing #####1.6
            rms2 = rms
            sendRmsToNextion(rms2)
            #defineTrigger(rms2) 
            if vox_enable.get():
                    state_2 = vox_delay.get() 
                    inverseGpioState(state_2)
                   #  if rms > vox_threshold.get():   # is it loud enough?
                    trigger =vox_threshold.get()      # MODIF 
                    ptt_On_Off(rms)       # MODIF  
                    agcMicGain(rms2)  #vox_threshold.get() # vox_delay.get()  Yes, reset the decay value (wont unkey for N samples)
                    if ((ptt == False)  and (transmit_enable == True) and (pttAudio ==EtatA)):   # MODIF  
                         ptt = True              # Set it
                         showPTTState(0)         # Update the UI (turn transmit button red, etc)
                         pttOn()                 # MODIF
                    elif ptt == True and pttAudio==EtatB :  #  MODIF   # Are we too soft and transmitting?
                         ptt = False             # Unkey
                         showPTTState(1)         # Update the UI
                         pttOff()                # MODIF
 
            #ptt_On_Off(rms) 
            if ptt != lastPtt and pttAudio== EtatB :  # MODIF
                usrp = 'USRP'.encode('ASCII') + struct.pack('>iiiiiii',usrpSeq, 0, ptt, 0, USRP_TYPE_VOICE, 0, 0) + audio
                sendto(usrp)
                usrpSeq = usrpSeq + 1
            lastPtt = ptt
            if ptt == True and pttAudio == EtatA :   # MODIF
                usrp = 'USRP'.encode('ASCII') + struct.pack('>iiiiiii',usrpSeq, 0, ptt, 0, USRP_TYPE_VOICE, 0, 0) + audio
                sendto(usrp)
                usrpSeq = usrpSeq + 1
                # print (int(rms/100),int(rms/200))  # debug 
                audio_level.set(int(rms/niveaux)) # MODIF
                bartxrx(int(rms/100),1)  # MODIF 
        except:
...
def makeVoxSettingsFrame( parent ):
    ...
    Checkbutton(voxSettings, text = STRING_DONGLE_MODE, variable=dongle_mode, command=lambda: gpioOnOff(~gpios), fg=uc_text_color, bg = uc_background_color, bd = 0, highlightthickness = 0).grid(column=1, row=1, sticky=W)
#    Checkbutton(voxSettings, text = STRING_GPIO_MODE , variable=dongle_mode,command=lambda: gpioOnOff(~gpios), fg=uc_text_color, bg = uc_background_color, bd = 0, highlightthickness = 0).grid(column=1, row=1, sticky=W)
...
#    dongle_mode = makeTkVar(IntVar, config.get('DEFAULTS', "dongleMode").split(None)[0]) # MODIF
    dongle_mode = makeTkVar(IntVar, config.get('DEFAULTS', "gpioOnOff").split(None)[0])
